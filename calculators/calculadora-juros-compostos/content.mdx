...
Esta calculadora aplica a fórmula dos juros compostos M = C × (1 + i)ⁿ + A × [((1 + i)ⁿ − 1) ÷ i] para mostrar o montante final, o total investido e o total em juros, com aporte inicial e mensal. A conversão de taxa anual para mensal é composta: 12,6825% ao ano equivale a 1% ao mês. É um cálculo matemático nominal, sem tabelas oficiais.

| Item | Valor |
| --- | --- |
| Conversão de taxa anual para mensal | (1 + taxa anual)^(1/12) − 1 |
| Exemplo de equivalência | 12,6825% ao ano = 1% ao mês |
| Resultado exibido | valor nominal bruto (sem IR, IOF ou inflação) |
...
Por exemplo, **12,6825% ao ano equivalem a 1% ao mês** — e não 12% ao ano, como sugeriria a conta simples de multiplicar por 12. Por isso, ao escolher "Anual" no campo de período da taxa, esta calculadora faz a conversão composta automaticamente e mostra a **taxa mensal efetiva** usada no cálculo.
...
> Esta é uma simulação de juros compostos de caráter informativo. Ela mostra o valor nominal bruto e não substitui orientação financeira. Rendimentos reais de qualquer investimento podem variar e sofrer Imposto de Renda, IOF e taxas. Consulte um profissional antes de decidir.
