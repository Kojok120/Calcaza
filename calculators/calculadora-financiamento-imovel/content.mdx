...
Esta calculadora simula o financiamento imobiliário pelos sistemas SAC (parcela decrescente) e Tabela Price (parcela fixa), com a taxa anual convertida em taxa mensal equivalente: i = (1 + taxa anual)^(1/12) − 1. A simulação segue os conceitos do Banco Central e da Caixa e não inclui CET, seguros (MIP/DFI) nem tarifas.

| Item | Valor |
| --- | --- |
| Sistema SAC | amortização constante, parcela decrescente |
| Tabela Price | parcela fixa do início ao fim |
| Conversão da taxa | i = (1 + taxa anual)^(1/12) − 1 |
| Entrada usual | em torno de 20% do valor do imóvel |
| Não incluído | CET, seguros MIP e DFI, taxa de administração |
